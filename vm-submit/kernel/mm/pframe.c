...
pframe_get(struct mmobj *o, uint32_t pagenum, pframe_t **result)
{
        ...
        // if the page frame is found and it is busy, wait until it is not busy
...
pframe_pin(pframe_t *pf)
{
        ...
        dbg(DBG_PRINT, "(GRADING3A 1.b)\n");
        int one_value;
        // check if the page frame is already pinned
        if (!pframe_is_pinned(pf)) {
                // remove from the current list and add to the pinned list
                list_remove(&(pf->pf_link));
                list_insert_tail(&pinned_list, &(pf->pf_link));

                // update the counts of pinned frames and allocated
                one_value = 1;
                npinned += one_value;
                nallocated -= one_value;

                dbg(DBG_PRINT, "(GRADING3B)\n");
        }

        // increment the pin count in either case
        pf->pf_pincount ++;
        dbg(DBG_PRINT, "(GRADING3B)\n");
}
...
pframe_unpin(pframe_t *pf)
{
        ...
        if (pf->pf_pincount == init_value) {
                // perform list operations to move the frame
                list_remove(&(pf->pf_link));
                list_insert_tail(&alloc_list, &(pf->pf_link));
