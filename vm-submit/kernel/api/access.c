...
addr_perm(struct proc *p, const void *vaddr, int perm)
{
        ...
        // look up the vmarea that might contain the given virtual address
        if (vm_area == NULL) {
                dbg(DBG_PRINT, "(GRADING3D)\n");
                return 0;
        }

        // check the vm area's permissions
...
{
        // calculate the start and the end of the range
        uintptr_t start = (uintptr_t)avaddr;
        uintptr_t end = (uintptr_t)avaddr + len;
        end = (uintptr_t)PAGE_ALIGN_UP(end);

        // iterate over each page in the range
        uintptr_t current_addr = start;
        while (current_addr < end) {
                // check if the current page address has the required permissions
                if (!addr_perm(p, (const void *)current_addr, perm)) {
                        dbg(DBG_PRINT, "(GRADING3B)\n");
                        return 0; 
                }
        ...
        // all pages in the range have permissions
        dbg(DBG_PRINT, "(GRADING3B)\n");
        return 1;
}
