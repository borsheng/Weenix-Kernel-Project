...
sys_read(read_args_t *arg)
{
        ...
        // allocate a temporary buffer in the kernel to store user data
        void *kernel_buffer = page_alloc();

        // validate the input argument pointer
        if (arg == NULL) {
        ...
        copy_status = copy_from_user(&kernel_read_args, arg, sizeof(read_args_t));
        if (copy_status < 0) {
                curthr->kt_errno = -copy_status;
                dbg(DBG_PRINT, "(GRADING3B)\n");
                return -1;
        }

        // read data from the file descriptor into the kernel buffer
        bytes_read = do_read(kernel_read_args.fd, kernel_buffer, kernel_read_args.nbytes);
        if (bytes_read < 0) {
        ...
        // copy the read data from the kernel buffer back to user space
        copy_result = copy_to_user(kernel_read_args.buf, kernel_buffer, bytes_read);
        if (copy_result < 0) {
...
sys_write(write_args_t *arg)
{
        ...
        copy_status = copy_from_user(&kernel_write_args, arg, sizeof(write_args_t));
        if (copy_status < 0) {
                curthr->kt_errno = -copy_status;
                dbg(DBG_PRINT, "(GRADING3B)\n");
                return -1;
        ...
        // copy data from the user space buffer to the temporary kernel buffer
        ...
        // attempt to write the data from the kernel buffer to the specified file descriptor
        bytes_written = do_write(kernel_write_args.fd, kernel_buffer, kernel_write_args.nbytes);
        if (bytes_written < 0) {
...
sys_getdents(getdents_args_t *arg)
{
        ...
        int num_entries;
        int i;
        getdents_args_t kernel_getdents_args;
        ...
        // struct to store the arguments copied from user space
        copy_status = copy_from_user(&kernel_getdents_args, arg, sizeof(getdents_args_t));
        if (copy_status < 0) {
                curthr->kt_errno = -copy_status;
                dbg(DBG_PRINT, "(GRADING3B)\n");
                return -1;
        ...
        // allocate a temporary buffer in the kernel to hold data read from the file
        kernel_dirent = (dirent_t *)kmalloc(sizeof(dirent_t));
        if (!kernel_dirent) {
                curthr->kt_errno = ENOMEM;
                dbg(DBG_PRINT, "(GRADING3B)\n");
                return -1;
        ...
        i = 0;
        num_entries = kernel_getdents_args.count / sizeof(dirent_t);
        bytes_read = 0;

        while (i < num_entries) {
                // read a directory entry
                read_result = do_getdent(kernel_getdents_args.fd, kernel_dirent);
                if (read_result < 0) {
                        kfree(kernel_dirent);
                ...
                // copy the read directory entry back to user space
                copy_result = copy_to_user((void *)((uintptr_t)kernel_getdents_args.dirp + bytes_read), kernel_dirent, read_result);
                if (copy_result < 0) {
                        kfree(kernel_dirent);
                        curthr->kt_errno = -copy_result;
                        dbg(DBG_PRINT, "(GRADING3B)\n");
