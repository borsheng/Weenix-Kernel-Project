...
do_brk(void *addr, void **ret)
{
    ...
    if (iaddr < (uint32_t) curproc->p_start_brk || iaddr > USER_MEM_HIGH) {
    ...
    uint32_t new_vfn = (iaddr - 1) / PAGE_SIZE + 1;
    uint32_t current_vfn = ((uint32_t) curproc->p_brk - 1) / PAGE_SIZE + 1;

    if (new_vfn != current_vfn) {
        
        if (iaddr > (uint32_t) curproc->p_brk) {
            
            if (!vmmap_is_range_empty(curproc->p_vmmap, current_vfn, new_vfn - current_vfn)) {
            	dbg(DBG_PRINT, "(GRADING3D)\n");
                return -ENOMEM;
            }
            vmarea_t *vmarea = vmmap_lookup(curproc->p_vmmap, current_vfn - 1);
            if (vmarea) {
            	uint32_t temp = vmarea->vma_end;
                vmarea->vma_end = new_vfn;
                dbg(DBG_PRINT, "(GRADING3D)\n");
            } else {
                
                vmmap_map(curproc->p_vmmap, NULL, current_vfn, new_vfn - current_vfn,
        	...
        	vmarea_t *vmarea = vmmap_lookup(curproc->p_vmmap, current_vfn - 1);
            uint32_t temp = vmarea->vma_end;
            vmmap_remove(curproc->p_vmmap, new_vfn, current_vfn - new_vfn);
            dbg(DBG_PRINT, "(GRADING3D)\n");
        }
        dbg(DBG_PRINT, "(GRADING3D)\n");
    }

    // Update process's break
    curproc->p_brk = addr;
    *ret = curproc->p_brk;
    dbg(DBG_PRINT, "(GRADING3D)\n");
    return 0;
