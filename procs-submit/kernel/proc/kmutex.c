...
kmutex_init(kmutex_t *mtx)
{
        ...
        kthread_t* noMutex=NULL;
        mtx->km_holder = noMutex;
...
kmutex_lock(kmutex_t *mtx)
{
        ...
        kthread_t* noMutex=mtx->km_holder;
        if(noMutex == NULL){
...
kmutex_lock_cancellable(kmutex_t *mtx)
{
        ...
        if(noMutex == NULL){
                ...
                int schedSleep=sched_cancellable_sleep_on(&(mtx->km_waitq));
                noMutex = NULL;
                dbg(DBG_PRINT, "(GRADING1C)\n");
                return schedSleep;
        }
...
kmutex_unlock(kmutex_t *mtx)
{
        ...
        int isEmpty=sched_queue_empty(&(mtx->km_waitq));
        if(isEmpty){
                isEmpty=1;
                mtx->km_holder = NULL;
                dbg(DBG_PRINT, "(GRADING1C)\n");
                ...
                kthread_t * holders=sched_wakeup_on(&(mtx->km_waitq));
                isEmpty=1;
                mtx->km_holder = holders;
                dbg(DBG_PRINT, "(GRADING1C)\n");
        }
