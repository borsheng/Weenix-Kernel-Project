...
sched_cancellable_sleep_on(ktqueue_t *q)
{
        ...
        /* check if thread's cancelled every time got back from sched_switch */
        if(curthr->kt_cancelled){
                return -EINTR;
        }
        return 0;
...
sched_switch(void)
{
        ...
        /*
             ...
             1.sched_cancellable_sleep_on() or sched_sleep_on() is called
             2.
         */
        ...
        //         curthr = thr;
        //         curproc = thr->kt_proc;
        //         context_switch(&curthr->kt_ctx, &thr->kt_ctx);
        //         //intr_enable();
        //         intr_setipl(IPL_LOW);
        // }
        //run queue is empty
        //must use while because parent process may invoke sched_sleep_on multiple times 
        //to wait for one of its child process to die
...
sched_make_runnable(kthread_t *thr)
{
        ...
        /* 
            a thread can be made runnable when:
            (1) wakeup_on is called
            (2) idle thread make init thread runnable by calling this function

            there'll be a queue of runnable threads with kt_state = KT_RUN, which one get allowed 
            to run is decided by sched_switch( ) which deque the first thread and call context_switch( )
            to make eip point to the code where it left off 
        ...
        intr_setipl(prev_ipl);
        // curthr = thr;    //maybe it is updated by sched_switch?
        // intr_enable();
        dbg(DBG_PRINT, "(GRADING1A)\n");
}
